...
function formatSchedule(startTime, endTime, includeDate = false) {
  const start = new Date(startTime);
  const end = new Date(endTime);

  if (includeDate) {
    return `${start.toLocaleDateString('ja-JP')} ${start.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })} - ${end.toLocaleDateString('ja-JP')} ${end.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}`;
  } else {
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const includeDate = isCoopMode; // サーモンランなら日付も含む
      const timeRange = formatSchedule(firstInfo.start_time, firstInfo.end_time, includeDate);
